...
const app = express();
mongoose.connect('mongodb+srv://paras:paras@paras-db.j8cs9.mongodb.net/paras-db?retryWrites=true&w=majority', { useCreateIndex: true, useNewUrlParser: true, useUnifiedTopology: true });
...
app.post('/language', (req, res) => {
    ...
    Language.findOneAndUpdate(query, { "$set": { "name": name, "code": code}}, {upsert: true,
...
app.post('/country', (req, res) => {
    ...
    Country.findOneAndUpdate(query, existing, {upsert: true,
        useFindAndModify: false}, (err, doc) => {
        if (err) return res.send(500, {error: err});
        return res.send('Succesfully saved.');
    });       
...
app.post('/updateMovie', async (req, res) => {
    // get data from the view and add it to mongodb
    try {
        const newMovie = await Movie.update({_id: req.body._id}, req.body, {upsert: true});
        if(!newMovie) throw err;
